...
def set_endpoints(endpoints: deque):
    cache.set(FIAT_PROVIDERS_CACHE, endpoints)


def get_current_endpoint():
    endpoints = cache.get(FIAT_PROVIDERS_CACHE)
    if not endpoints:
        endpoints = deque(settings.FIAT_RPC_ENDPOINTS)
...
def check_bnb_response_time(w3, time_sec):
    current_counter = cache.get(FIAT_RESPONSE_TIME_COUNTER_CACHE, 0)
    if time_sec >= 2.8:
        current_counter += 1
    else:
        current_counter = 0
    ...
    cache.set(FIAT_RESPONSE_TIME_COUNTER_CACHE, current_counter)
